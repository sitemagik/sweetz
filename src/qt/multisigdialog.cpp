...
void MultisigDialog::on_createAddressButton_clicked()
{
    ...
    CsweetcoinAddress address(scriptID);
...
void MultisigDialog::on_saveMultisigAddressButton_clicked()
{
    ...
    if(!wallet->HaveCScript(scriptID))
        wallet->AddCScript(script);
    if(!wallet->mapAddressBook.count(CsweetcoinAddress(address).Get()))
        wallet->SetAddressBookName(CsweetcoinAddress(address).Get(), label);
}
    ...
    for(int i = 0; i < ui->outputs->count(); i++)
    {
        ...
        if(entry)
        {
            if(entry->validate())
            {
                ...
                CsweetcoinAddress address(recipient.address.toStdString());
...
void MultisigDialog::on_transaction_textChanged()
{
    ...
    BOOST_FOREACH(const CTxOut& txout, tx.vout)
    {
        ...
        CsweetcoinAddress address(addr);
