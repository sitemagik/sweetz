    ...
    model(0)
{
    ...
    // Coin Control
    ui->lineEditCoinControlChange->setFont(GUIUtil::sweetcoinAddressFont());
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    {
        formatted.append(tr("<b>%1</b> to %2 (%3)").arg(sweetcoinUnits::formatWithUnit(sweetcoinUnits::BTC, rcp.amount), Qt::escape(rcp.label), rcp.address));
    }
    ...
    case WalletModel::AmountWithFeeExceedsBalance:
        QMessageBox::warning(this, tr("Send Coins"),
            ...
            arg(sweetcoinUnits::formatWithUnit(sweetcoinUnits::BTC, sendstatus.fee)),
            QMessageBox::Ok, QMessageBox::Ok);
        break;
...
void SendCoinsDialog::handleURI(const QString &uri)
{
    ...
    if(!GUIUtil::parsesweetcoinURI(uri, &rv))
    {
        return;
    }
    pasteEntry(rv);
...
void SendCoinsDialog::setBalance(qint64 balance, qint64 stake, qint64 unconfirmedBalance)
{
    ...
    ui->labelBalance->setText(sweetcoinUnits::formatWithUnit(unit, balance));
}
...
void SendCoinsDialog::updateDisplayUnit()
{
    if(model && model->getOptionsModel())
    {
        ...
        ui->labelBalance->setText(sweetcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), model->getBalance()));
    }
}
...
void SendCoinsDialog::coinControlChangeChecked(int state)
{
    ...
    {
        if (state == Qt::Checked)
            CoinControlDialog::coinControl->destChange = CsweetcoinAddress(ui->lineEditCoinControlChange->text().toStdString()).Get();
        else
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        CsweetcoinAddress addr = CsweetcoinAddress(text.toStdString());
