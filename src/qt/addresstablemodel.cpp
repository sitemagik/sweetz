...
QVariant AddressTableModel::data(const QModelIndex &index, int role) const
{
    ...
    else if (role == Qt::FontRole)
    {
        ...
        {
            font = GUIUtil::sweetcoinAddressFont();
        }
        return font;
    }
...
bool AddressTableModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        {
        case Label:
            wallet->SetAddressBookName(CsweetcoinAddress(rec->address.toStdString()).Get(), value.toString().toStdString());
            ...
            {
                {
                    LOCK(wallet->cs_wallet);
                    // Remove old entry
                    wallet->DelAddressBookName(CsweetcoinAddress(rec->address.toStdString()).Get());
                    // Add new entry with new address
                    wallet->SetAddressBookName(CsweetcoinAddress(value.toString().toStdString()).Get(), rec->label.toStdString());
                }
...
{
    // Update address book model from sweetcoin core
    beginResetModel();
    priv->refreshAddressTable();
    endResetModel();
}
...
QString AddressTableModel::addRow(const QString &type, const QString &label, const QString &address)
{
    ...
    if(type == Send)
    {
        ...
        {
            LOCK(wallet->cs_wallet);
            if(wallet->mapAddressBook.count(CsweetcoinAddress(strAddress).Get()))
            {
    ...
    else if(type == Receive)
    {
        ...
        strAddress = CsweetcoinAddress(newKey.GetID()).ToString();
    }
    else
    {
        return QString();
    ...
    {
        LOCK(wallet->cs_wallet);
        wallet->SetAddressBookName(CsweetcoinAddress(strAddress).Get(), strLabel);
    }
...
QString AddressTableModel::labelForAddress(const QString &address) const
{
    {
        LOCK(wallet->cs_wallet);
        CsweetcoinAddress address_parsed(address.toStdString());
