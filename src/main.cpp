                                 ...
                                 bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal sweetcoin miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // paycoin: fees are not collected by miners as in sweetcoin
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Time 11/11/2016 World destroyed by low IQ Americans";
        CTransaction txNew;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "block")
    {
        ...
        uint256 swzhash = block.GetHash();
        printf("received block %s\n", swzhash.ToString().substr(0,20).c_str());

        CInv inv(MSG_BLOCK, swzhash);
        pfrom->AddInventoryKnown(inv);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("sweetcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadsweetcoinMiner(void* parg);

static bool fGeneratesweetcoins = false;
...
void sweetcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGeneratesweetcoins || fProofOfStake)
    {
        if (fShutdown)
            return;
        ...
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGeneratesweetcoins) && !fProofOfStake)
                return;
        }
...
void static ThreadsweetcoinMiner(void* parg)
{
    ...
    try
    {
        ...
        sweetcoinMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadsweetcoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadsweetcoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadsweetcoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void Generatesweetcoins(bool fGenerate, CWallet* pwallet)
{
    fGeneratesweetcoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGeneratesweetcoins = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d sweetcoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadsweetcoinMiner, pwallet))
                printf("Error: NewThread(ThreadsweetcoinMiner) failed\n");
            Sleep(10);
        }
    }
}
