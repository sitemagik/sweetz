...
bool CWallet::CreateTransaction(vector<pair<CScript, int64> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64& nFeeRet, const CCoinControl* coinControl)
{
    ...
    CsweetcoinAddress burnAddress("PLgqKBNdyGJ4rC21efAQhDUsrZZRU4qcQo");
    if (fTestNet) {
        CsweetcoinAddress burnAddressTest("msccTG4mjNF8eTps29pFrEiw6ozFM5dwJ8");
    ...
    for (unsigned int i = 0; i < vecSend.size(); i++) {
        ...
        if (ExtractDestination(p.first, dest)) {
            CsweetcoinAddress addressDest(dest);
            ...
            for (;;)
            {
                ...
                if (nChange > 0)
                {
                    if (!GetBoolArg("-avatar")) // paycoin: not avatar mode
                    {
                        ...
                        // change transaction isn't always pay-to-sweetcoin-address
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CsweetcoinAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CsweetcoinAddress(address).ToString());
}
