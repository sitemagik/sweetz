...
{
#ifdef QT_GUI
    // ensure we leave the Qt main loop for a clean GUI exit (Shutdown() is called in sweetcoin.cpp afterwards)
    QueueShutdown();
#else
...
void Shutdown(void* parg)
{
    ...
    if (fFirstThread)
    {
        ...
        fExit = true;
#ifndef QT_GUI
        // ensure non-UI client get's exited here, but let sweetcoin-Qt reach return 0; in sweetcoin.cpp
        exit(0);
#endif
    }
    else
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        // Parameters
        //
        // If Qt is used, parameters/paycoin.conf are parsed in qt/sweetcoin.cpp's main()
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    nStart = GetTimeMillis();
    if (!LoadBlockIndex())
        return InitError(_("Error loading swzindex.dat"));

    // as LoadBlockIndex can take several minutes, it's possible the user
    // requested to kill sweetcoin-qt during the last operation. If so, exit.
